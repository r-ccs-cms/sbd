...
void append_comma_separated(const std::string& text,
...
void print_options(std::ostream& output,
...
int main(int argc, char** argv) {
  ...
  int exit_code = 0;
  try {
    const CliOptions cli = parse_options(argc, argv);
    if(cli.help) {
      if(rank == 0) print_usage(std::cout);
    } else {
      const std::size_t load_b_size = cli.wavefunction_shards == 0
          ? static_cast<std::size_t>(size) : cli.wavefunction_shards;
      if(load_b_size == 0 || load_b_size > static_cast<std::size_t>(size))
        throw std::invalid_argument(
            "--wavefunction-shards must be in [1,MPI size]");
      ...
      sbd::FCIDump fcidump;
      if(rank == 0) fcidump = sbd::LoadFCIDump(cli.fcidump_path);
      sbd::MpiBcast(fcidump, 0, world);
      ...
      sbd::SetupIntegrals(fcidump, integral_norb, nelec, scalar_integral,
                          one_integrals, two_integrals);
      if(integral_norb <= 0)
        throw std::runtime_error("FCIDUMP has no positive NORB");
      const std::size_t norb = static_cast<std::size_t>(integral_norb);
      sbd::det_vector<std::size_t>::init_elem_size(
          (2 * norb + cli.bit_length - 1) / cli.bit_length);
      sbd::det_vector<std::size_t, sbd::det_kind::half>::init_elem_size(
          (norb + cli.bit_length - 1) / cli.bit_length);

      sbd::det_vector<std::size_t> parents;
      std::vector<double> coefficients;
      if(load_rank) {
        sbd::load_basis_from_files(cli.determinant_files, parents,
                                   cli.bit_length, 2 * norb, load_b_comm);
      ...
      if(rank == 0)
        print_options(std::cout, cli, calculation_id, load_b_size, size);

      std::vector<sbd::gdb::BatchRequest> batches(cli.batch_count);
      ...
      sbd::gdb::evaluate_statistical_batches<double, double>(
          parents, coefficients, scalar_integral, one_integrals,
          two_integrals, options, batches, world,

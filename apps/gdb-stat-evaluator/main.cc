...
int main(int argc, char** argv) {
  ...
  int exit_code = 0;
  try {
    const auto cli = gdb_stat_evaluator::parse_options(argc, argv);
    if(cli.help) {
      if(rank == 0) gdb_stat_evaluator::print_usage(std::cout);
    } else {
      ...
      if(rank == 0)
        gdb_stat_evaluator::print_options(std::cout, cli, calculation_id, load_b_size, size);

/** CLI options and argument handling for the caop-stat-evaluator application. */
#ifndef SBD_APPS_CAOP_STAT_EVALUATOR_CLI_OPTIONS_H
#define SBD_APPS_CAOP_STAT_EVALUATOR_CLI_OPTIONS_H
...
inline CliOptions parse_options(int argc, char** argv) {
  ...
  for(int index = 1; index < argc; ++index) {
    ...
    if(argument == "--help" || argument == "-h") {
      options.help = true;
    } else if(argument == "--hamfile") {
      options.hamiltonian_path = require_value(argc, argv, index);
    } else if(argument == "--h-comm-size" || argument == "--h_comm_size") {
      options.h_comm_size = parse_unsigned_decimal<std::size_t>(require_value(argc,argv,index), "--h-comm-size");
    } else if(argument == "--sites") {
      options.sites = parse_unsigned_decimal<std::size_t>(require_value(argc, argv, index), "--sites");
    } else if(argument == "--detfiles" || argument == "--basisfiles") {
  ...
  if(options.h_comm_size == 0) throw std::invalid_argument("--h-comm-size must be positive");
  if(options.sites == 0) throw std::invalid_argument("--sites must be positive");
  if(options.hamiltonian_path.empty())
    throw std::invalid_argument("--hamfile is required");
...
inline void print_usage(std::ostream& output) {
  output
      << "usage: caop-stat-evaluator --hamfile FILE --sites L --detfiles FILE[,FILE...]\n"
         "       --loadname PREFIX --output FILE [options]\n"
         "options:\n"
         "  --h-comm-size N                 (default: 1; must divide MPI size)\n"
         ...
         "  --wavefunction-shards N         (saved b size; default: current b size)\n"
         "  --seed N                        (default: 0)\n"
...
inline void print_options(std::ostream& output,
  ...
  output << "# h size: " << options.h_comm_size << '\n';
  output << "# b size: " << mpi_size/options.h_comm_size << '\n';
  output << "# sites: " << options.sites << '\n';
  output << "# Hamiltonian: " << options.hamiltonian_path << '\n';
